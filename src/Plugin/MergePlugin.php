...
namespace ByKit\Laravel\Plugin;
...
class MergePlugin implements PluginInterface, EventSubscriberInterface
{
     ...
     * @throws \ByKit\Laravel\Exceptions\MissingFileException when required and a pattern returns no
     *      results
     */
    protected function mergeFiles(array $patterns, $required = false)
    {
        ...
        $files = array_map(
            static function ($files, $pattern) use ($required) {
                if ($required && !$files) {
                    throw new \ByKit\Laravel\Exceptions\MissingFileException(
